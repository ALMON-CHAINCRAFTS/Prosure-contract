...
contract insure {
    ...
    event NewInsure(
        ...
        uint totalCoverCreated,
        address creatorAddress,
        RiskLevel _risklevel,
        uint creationTime,
        uint _protocolID
    );
    event AddOnExistingInsure(
        string protocolName,
        string protocolDomain,
        uint coverAdded,
        address creatorAddress,
    ...
    function createNewInsure(
        ...
        Proto.RiskAsessors[msg.sender].initialCoverCreationDate = block
            .timestamp;
    ...
    function createOnExistinginsure(uint _id, uint _coverAmount) external {
    ...
    function buyCover(uint _id, uint _coverPeriod, uint _coverAmount) external {
        Protocol storage Proto = AllProtocols[_id];
        RiskLevel levelOfRisk = Proto.risklevel;
        uint coverToPay = calculateCover(
            levelOfRisk,
            _coverPeriod,
            _coverAmount
        );
        ...
        require(
            Proto.coverLeft >= _coverAmount,
            "Not enough cover left for you"
        );
        Proto.UsersData[msg.sender].totalCoverBought += _coverAmount;
        Proto.coverLeft -= _coverAmount;
        Proto.UsersData[msg.sender].dateBought +=
            block.timestamp +
            (_coverPeriod * 1 days);
    ...
    function userGetClaim(uint _idOfClaimRequests) external {
    ...
    function riskAssessorGetsClaimBack(uint _idOfClaimRequests) external {
        (uint insureId, uint _refund) = IGovernace(goveranceAddress)
            .riskAssessorWithdrawInsurance(_idOfClaimRequests);
    ...
    function riskassessorWithdrawProfit(uint _id) external {
        Protocol storage Proto = AllProtocols[_id];
    ...
    function withDrawFee(address _feeAddress) external {
        onlyAdmin();
    ...
    function riskassessorWithdrawCover(uint _id) external {
        Protocol storage Proto = AllProtocols[_id];
        uint depositDate = Proto
            .RiskAsessors[msg.sender]
            .initialCoverCreationDate;
        ...
        uint _riskAssessorCover = Proto
            .RiskAsessors[msg.sender]
            .totalCoverProvided;
        uint withdrawable = (_riskAssessorCover * _totalCoverWithdrawable) /
            _totalCover;
    ...
    function getAllUsersOfProtocol(
        uint _id
    ...
    function getAllProtocolData()
        public
        view
        returns (ProtocolData[] memory protocolData)
    {
    ...
    function calculateCover(
        ...
        uint _coverPeriod,
        uint _coverAmount
    ) public pure returns (uint cover) {
        if (_riskLevel == RiskLevel.very_low) {
            cover =
                ((VERY_LOW + 25) * (_coverPeriod * _coverAmount)) /
                (uint256(PERCENTAGE) * YEAR);
        } else if (_riskLevel == RiskLevel.low) {
            cover =
                ((LOW + 25) * (_coverPeriod * _coverAmount)) /
                (uint256(PERCENTAGE) * YEAR);
        } else if (_riskLevel == RiskLevel.medium) {
            cover =
                ((MEDIUM + 25) * (_coverPeriod * _coverAmount)) /
                (uint256(PERCENTAGE) * YEAR);
        } else if (_riskLevel == RiskLevel.high) {
            cover =
                ((HIGH + 25) * (_coverPeriod * _coverAmount)) /
                (uint256(PERCENTAGE) * YEAR);
        } else {
            cover =
                ((VERY_HIGH + 25) * (_coverPeriod * _coverAmount)) /
                (uint256(PERCENTAGE) * YEAR);
        }
    }
    ...
    function deposit(uint _amount) internal returns (bool sent) {
        amountMustBeGreaterThanZero(_amount);
        sent = IERC20(tokenAddress).transferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function withdraw(address _to, uint _amount) private returns (bool sent) {
        amountMustBeGreaterThanZero(_amount);
